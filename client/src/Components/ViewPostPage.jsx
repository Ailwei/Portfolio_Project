...
const ViewPosts = ({setSelectedPost}) => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get(`https://13.53.199.9/get_posts?page=${currentPage}`);
      ...
      const commentsResponse = await Promise.all(postIds.map(postId => axios.get(`https://13.53.199.9/get_comments/${postId}`)));
  ...
  const fetchReactions = async (postIds) => {
    try {
      const reactionsResponse = await Promise.all(postIds.map(postId =>
        axios.get(`https://13.53.199.9/posts/${postId}/reactions`)
      ));
  ...
  const handleReaction = async (postId) => {
    try {
      ...
      await axios.post(`https://13.53.199.9/reactions`,  
        {
