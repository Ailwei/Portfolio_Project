...
const ViewUserProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/view_user_profile/${userId}`, {
          headers: {
  ...
  const handleBlockUserorUnblockUser = async () => {
    try {
      const response = profile.blocked ? 
        await axios.post(`http://localhost:5000/unblock_user`, { user_id: userId, group_id: groupId }, { headers: { Authorization: `Bearer ${authToken}` } }) :
        await axios.post(`http://localhost:5000/block_user`, { user_id: userId, group_id: groupId }, { headers: { Authorization: `Bearer ${authToken}` } });
  ...
  const handleSendMessage = async () => {
    try {
      const response = await axios.post(`http://localhost:5000/send_message/${userId}`, { user_id: userId, content: messageContent }, { headers: { Authorization: `Bearer ${authToken}` } });
