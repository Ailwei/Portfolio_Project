...
const ViewUserProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await axios.get(`https://13.53.199.9/view_user_profile/${userId}`, {
          headers: {
  ...
  const handleBlockUserorUnblockUser = async () => {
    try {
      const response = profile.blocked ? 
        await axios.post(`https://13.53.199.9/unblock_user`, { user_id: userId, group_id: groupId }, { headers: { Authorization: `Bearer ${authToken}` } }) :
        await axios.post(`https://13.53.199.9/block_user`, { user_id: userId, group_id: groupId }, { headers: { Authorization: `Bearer ${authToken}` } });
  ...
  const handleSendMessage = async () => {
    try {
      const response = await axios.post(`https://13.53.199.9/send_message/${userId}`, { user_id: userId, content: messageContent }, { headers: { Authorization: `Bearer ${authToken}` } });
  ...
  const fetchFollowedUsers = async () => {
    try {
      const response = await axios.get('https://13.53.199.9/followed_users', {
        headers: {
          Authorization: `Bearer ${authToken}`
        }
      });
  ...
  const handleFollow = async () => {
    ...
    if (userId) {
      ...
      if (followedUsers.includes(profile.userId)) {
        await axios.post(`https://13.53.199.9/unfollow/${userId}`, {}, {
          headers: {
            Authorization: `Bearer ${authToken}`
          }
        });
        ...
        await axios.post(`https://13.53.199.9/follow/${userId}`, {}, {
          headers: {
            Authorization: `Bearer ${authToken}`
          }
        });
